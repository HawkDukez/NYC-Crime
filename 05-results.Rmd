...
From the histograms of boroughs and the precinct map in NYC, we can see the different pattern of crimes on different police precincts. In Manhattan, crime number of each precincts are relatively even (except for precinct 14), however, Bronx has a distribution with higher variance. We can use this chart to judge which district is suitable for more police force.
...
drug18 <- ggmap(nyc_map) + 
  geom_point(data=nyc_18_drugs, aes(x=Longitude,y=Latitude),size=.15,alpha=.3,color = 'red') +
  ggtitle("DANGEROUS DRUGS in 2018")

drug21 <- ggmap(nyc_map) + 
  geom_point(data=nyc_21_drugs, aes(x=Longitude,y=Latitude),size=.15,alpha=.3,color = 'red') +
...
larceny18 <- ggmap(nyc_map) + 
  geom_point(data=nyc_18_larceny, aes(x=Longitude,y=Latitude),size=.15,alpha=.3,,color='darkblue') +
  ggtitle("PETIT LARCENY in 2018")+
  theme(legend.title = element_blank())

larceny21 <- ggmap(nyc_map) + 
  geom_point(data=nyc_21_larceny, aes(x=Longitude,y=Latitude),size=.15,alpha=.3,,color='darkblue') +
...
From this two plot, we can find that larceny crimes are concentrated on the midtown and downtown, which around the central park. From 2018 to 2020, the larceny crimes on Manhattan downtown decrease a lot however the number of PETIT LARCENY in other areas has no big difference.
...
map2 <- ggmap(nyc_map) +  
  geom_point(data=nyc_20_drugs, aes(x=Longitude, y=Latitude), size=1, alpha=0.1) +
  ggtitle("DANGEROUS DRUGS in 2021")
...
map2 <- ggmap(nyc_map) +  
  geom_point(data=nyc_20_weapon, aes(x=Longitude,y=Latitude),size=0.5,alpha=0.1) +
