...
p2<-ggplot(cdf1,aes(DATE_OF_INTEREST, y = rate, group=category, color=category)) +
  geom_line() + xlab("") + ylab("") +
  theme(legend.title = element_blank())+
...
hate1<-hate%>%
  ...
  mutate(count=n())
p1<-ggplot(hate1,aes(fct_reorder(`Offense Category`,count),count))+
  geom_point()+
  coord_flip()+
  labs(x='Offense Category')+
  ggtitle('Popular offense category in recent 3 years')
ggplot(hate1,  mapping=aes(x='Offense Category',y=count,fill=`Offense Category`)) +
  geom_bar(stat="identity",width=0.5,position='stack',size=5)+
  coord_polar("y", start=0)+
  ggtitle("Crimes Percentage for Different Offense Category")+
...
hate%>%
  ...
  ggtitle('Numbers of hate crime in different types over recent years')
...
hate%>%
  ...
  geom_line(aes(group = `Bias Motive Description`)) +geom_point()+
  scale_x_date(date_breaks = "4 months", date_labels = "%b %Y") 
```
```{r}
hate%>%
