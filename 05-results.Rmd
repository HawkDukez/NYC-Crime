...
drug20 <- ggmap(nyc_map) + 
  geom_polygon(data=nyc_neighborhoods_df, aes(x=long, y=lat, group=group), color="white", fill=NA)+
  geom_point(data=nyc_20_drugs, aes(x=Longitude,y=Latitude),size=.5,alpha=.3,color = 'red') +
...
larceny20 <- ggmap(nyc_map) + 
  geom_polygon(data=nyc_neighborhoods_df, aes(x=long, y=lat, group=group), color="white", fill=NA)+
  geom_point(data=nyc_20_larceny, aes(x=Longitude,y=Latitude),size=.5,alpha=.3,,color='darkblue') +
...
ggplot(hate1)+
  ...
  scale_x_date(limits=as.Date(c("2019-01-01","2021-112-31")),date_breaks = "3 months", date_labels = "%b %Y")+
...
ggplot(hate1,aes(fct_reorder(`Offense Category`,count),count))+
  ...
  ggtitle('Popular offense category in recent 3 years')
```
```{r fig.height=6}
...
hate %>%group_by(Date, County) %>%
  ...
  scale_x_date(date_breaks = "3 months", date_labels = "%b %Y")+
  ggtitle('Hate crimes in different boroughs')
```
