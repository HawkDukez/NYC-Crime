...
pd_desc10_top <- top_n(pd_desc10,16)

pd_desc11 <- count(data11,PD_DESC)
pd_desc11_top <- top_n(pd_desc11,16)

pd_desc12 <- count(data12,PD_DESC)
pd_desc12_top <- top_n(pd_desc12,16)

pd_desc13 <- count(data13,PD_DESC)
pd_desc13_top <- top_n(pd_desc13,16)

pd_desc14 <- count(data14,PD_DESC)
pd_desc14_top <- top_n(pd_desc14,16)

pd_desc15 <- count(data15,PD_DESC)
pd_desc15_top <- top_n(pd_desc15,16)
...
```{r,fig.width = 8, fig.height = 5}
most_crime <- intersect(pd_desc11_top$PD_DESC, pd_desc12_top$PD_DESC)  %>% intersect(pd_desc13_top$PD_DESC)  %>% intersect(pd_desc14_top$PD_DESC) %>% intersect(pd_desc15_top$PD_DESC) %>% intersect(pd_desc16_top$PD_DESC) %>% intersect(pd_desc17_top$PD_DESC) %>% intersect(pd_desc18_top$PD_DESC) %>% intersect(pd_desc19_top$PD_DESC) %>% intersect(pd_desc20_top$PD_DESC)

most_crime_nyc <- nyc %>% filter(!is.na(PD_DESC)) %>% filter(PD_DESC %in% most_crime) %>% group_by(PD_DESC,ARREST_YEAR) %>%summarise(Count = length(PD_DESC))
...
ggplot(most_crime_nyc, aes(ARREST_YEAR, Count, color = PD_DESC)) + geom_line() +
    ggtitle("5 Most crimes in NYC") +
    theme_grey(16) +
    theme(legend.title = element_blank()) + 
    labs (x = "Year", y = "Count") + 
    theme(legend.key.size = unit(.5, 'cm'),  
          legend.key.width = unit(.5, 'cm'), legend.text = element_text(size=8)) +
    scale_x_date(limits = c(as.Date("2006-01-01"), as.Date("2020-01-01")),date_breaks = "1 year", date_labels = "%y")
```
...
ggplot(data = group_year) +
  ...
  coord_flip() + 
  theme(axis.text.x = element_text(angle = 45, vjust = 0.6, hjust=.5))
...
ggplot(pd_desc15_top,aes(x=n,y=reorder(PD_DESC,n)))+
  ...
  ylab("Crime")+
  xlab("Count")+
  ggtitle("30 most frequent crimes in 2015")

ggplot(pd_desc20_top,aes(x=n,y=reorder(PD_DESC,n)))+
  geom_point(color = 'blue')+
  ylab("Crime")+
  xlab("Count")+
  ggtitle("30 most frequent crimes in 2020")
